...
async function main() {
  const serviceAccount = require('./camp-lawton-staff-hub-firebase-adminsdk-fbsvc-439f443121.json');
  ...
  // Read markdown
  const mdContent = fs.readFileSync('staffHandbookCL.md', 'utf8');
  ...
  snapshot.forEach(doc => {
    ...
    if (handbookData[lookupTitle]) {
      ...
      paragraphs.forEach(p => {
        ...
        } else if (p.startsWith('1. ') || p.startsWith('2. ')) {
